...
# Configuration: prefer repo-local data/ directory but allow env overrides
...
# Load model (respect configured MODEL_PATH; fallback to Desktop if missing)
model_path = MODEL_PATH
if not os.path.exists(model_path):
    fb = os.path.join(os.path.expanduser("~"), "Desktop", "fraud_rnn_keras9.keras")
...
merchant2id = {}
try:
    if os.path.exists(TXN_TYPE_PATH):
        with open(TXN_TYPE_PATH, "rb") as f:
            txn_type2id = pickle.load(f)
    else:
        # fallback to Desktop location for dev env
        fb_txn = os.path.join(os.path.expanduser("~"), "Desktop", "txn_type2id.pkl")
        if os.path.exists(fb_txn):
            with open(fb_txn, "rb") as f:
                txn_type2id = pickle.load(f)
except Exception as e:
    print('Failed to load txn_type2id:', e)
try:
    if os.path.exists(MERCHANT_PATH):
        with open(MERCHANT_PATH, "rb") as f:
            merchant2id = pickle.load(f)
    else:
        fb_merch = os.path.join(os.path.expanduser("~"), "Desktop", "merchant2id.pkl")
        if os.path.exists(fb_merch):
            with open(fb_merch, "rb") as f:
                merchant2id = pickle.load(f)
except Exception as e:
    print('Failed to load merchant2id:', e)
...
try:
    # Prefer configured dataset path, fallback to Desktop dataset
    dataset_path = DATASETX_PATH
    if not os.path.exists(dataset_path):
        fb_ds = os.path.join(os.path.expanduser("~"), "Desktop", "datasetgeek.xlsx")
        if os.path.exists(fb_ds):
            dataset_path = fb_ds
...
print("Feature stats means:", feature_stats['means'])
print("Feature stats stds:", feature_stats['stds'])
...
# Load feature stats (from configured data dir or Desktop fallback)
...
try:
    bg_df = None
    bg_path = DATASETX_PATH
    if not os.path.exists(bg_path):
        fb_bg = os.path.join(os.path.expanduser("~"), "Desktop", "datasetgeek.xlsx")
        if os.path.exists(fb_bg):
            bg_path = fb_bg
...
def predict():
    ...
    ensure_transactions_table_and_columns()
    # Add prediction values to stored row so history can present them
    try:
        df_input['predicted_class'] = int(pred_class)
        df_input['probabilities'] = json.dumps(preds[0].tolist())
    except Exception as e:
        print('Error attaching prediction fields to df_input:', e)
    ...
    # Compute feature deviations using dynamic feature_stats
    ...
    try:
        if pred_class == 1:
            ...
            for feat in numeric_features_list:
                ...
                mean = float(feature_stats['means'].get(feat, 0.0))
                std = float(feature_stats['stds'].get(feat, 1.0))
                if std == 0:
                    std = 1.0
                z = (val - mean) / std
...
def history():
    ...
    for _, row in df_hist.iterrows():
        rowd = {col: (float(row[col]) if (col in ['amount','avg_amount_last_3_txns','avg_daily_transactions_30d','avg_transaction_amount_30d','amount_deviation_score'] and not pd.isna(row[col])) else (int(row[col]) if (col in ['is_new_device','location_change_flag','transactions_today','has_previous_history','predicted_class'] and not pd.isna(row[col])) else (row[col] if not pd.isna(row[col]) else None))) for col in df_hist.columns}
        # Convert probabilities column back to list if present
        if 'probabilities' in rowd and isinstance(rowd['probabilities'], str):
            try:
                rowd['probabilities'] = json.loads(rowd['probabilities'])
            except Exception:
                pass
        hist.append(rowd)
