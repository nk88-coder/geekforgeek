...
# load feature stats (means & stds) produced during training
feature_stats = None
feature_stats_path = os.path.join(os.path.expanduser("~"), "Desktop", "feature_stats.pkl")
try:
    ...
    try:
        if feature_stats is not None and pred_class == 1:
            ...
            # Top 2
            feature_deviations = [
                {'feature': d[0], 'z': float(d[1]), 'abs_z': float(d[2]), 'value': float(d[3]), 'mean': float(d[4]), 'std': float(d[5])}
                for d in deviations_sorted[:2]
            ]
            print('Top feature deviations:', feature_deviations)
    except Exception as e:
        feature_deviations = None
